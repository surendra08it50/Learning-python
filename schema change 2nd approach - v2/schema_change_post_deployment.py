...
def run_post_deployment(table_name_new, table_name_old):
    ...
    #         from
    #         (
    #         select distinct cast({col_name} as {dict_cols_temp[col_name]}), step, partitioned_by
    #         from public."{input_table_name}"
    #         where step not in ('PR', 'PS', 'PQ')
    #         ) t2
    #         where
    #         cast("{new_table_name}".step as FLOAT) = cast(t2.step as FLOAT) and
    #         "{new_table_name}".partitioned_by = t2.partitioned_by"""
    ...
    #         from
    #         (
    #         select distinct cast({col_name} as {dict_cols_temp[col_name]}), partitioned_by
    #         from public."{input_table_name}"
    #         ) t2
    #         where
    #         "{new_table_name}".partitioned_by = t2.partitioned_by"""
