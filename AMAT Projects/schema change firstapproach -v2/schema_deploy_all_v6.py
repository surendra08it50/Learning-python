...
output_col_list=str(ENV_VARIABLES['output_col_list']).replace('"',"").replace("[","").replace("]" ,"").replace("'",'').replace(".default",'."default"')
...
def schema_change(old_table):
    ...
    new_table_name = original_table_name + "_new"
    ...
    for col in new_cols_to_add:
        ...
        query_add_cols = f"""ALTER TABLE public."{original_table_name}" ADD COLUMN {col} {dict_cols_dev[col]} """
    ...
    ## #insert into new table from original table    
    query_insert_original_table = f"""insert into public."{new_table_name}" select * from public."{original_table_name}" """    
...
schema_change("OWPS_DRIFT_aic_owps_drift_op_21")
